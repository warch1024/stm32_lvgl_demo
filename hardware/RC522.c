...
void MF522SPI_Init(void) {
    ...
    // GPIO PC6 MOSI
    GPIO_InitStructure.GPIO_Pin = MF522_MOSI_PIN;              // PC6
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;              //
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;             //
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;         // 100MHz
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;               //
    GPIO_Init(MF522_RST_MOSI_MISO_GPIO, &GPIO_InitStructure);  //

    // GPIO PC8 MISO
    GPIO_InitStructure.GPIO_Pin = MF522_MISO_PIN;              // PC8
    ...
    // GPIO PD6 PD7 片选 时钟线
    GPIO_InitStructure.GPIO_Pin = MF522_SCK_PIN | MF522_NSS_PIN;  // PD6 PD7
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;                 //
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;            // 100MHz
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;                  //
    GPIO_Init(MF522_NSS_SCK_GPIO, &GPIO_InitStructure);           //
}
...
void MF522_Init(void) {
    ...
    // GPIO PC11 复位引脚
    GPIO_InitStructure.GPIO_Pin = MF522_RST_PIN;               // PC11
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;              //
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;             //
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;         // 100MHz
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;               //
...
// 返 回 值成功返回MI_OK
/*读取数据块, 16字节,
* 传入块地址（50卡一共64块，每块16字节，分为16个扇区，每个扇区4个块）
* 第4i块为控制块（i=1,2,3,4）存储密码a（6B），存取控制字（4B），密码b）
*/
